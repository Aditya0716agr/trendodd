...
// Less chromatic, more premium: no area fill, line only
const generateData = () => {
...
const AnimatedChart = ({ className = '' }: AnimatedChartProps) => {
  ...
  // Clean floating points, premium tooltip style
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white/90 dark:bg-card/95 backdrop-blur border border-border/70 p-3 rounded-xl shadow-xl min-w-[110px]">
          <p className="font-medium text-sm mb-1 text-gray-700 dark:text-gray-200">{label}</p>
          <div className="flex flex-col gap-1">
            <p className="text-sm">
              <span className="inline-block w-3 h-3 rounded-full bg-emerald-500 mr-2"></span>
              Yes: <span className="font-medium">{Number(payload[0].value).toLocaleString(undefined, { maximumFractionDigits: 2 })}¢</span>
            </p>
            <p className="text-sm">
              <span className="inline-block w-3 h-3 rounded-full bg-orange-400 mr-2"></span>
              No: <span className="font-medium">{Number(payload[1].value).toLocaleString(undefined, { maximumFractionDigits: 2 })}¢</span>
            </p>
          </div>
        </div>
      );
  ...
  return (
    <div
      className={`w-full max-w-2xl mx-auto my-6 glass-card shadow-2xl border-2 border-primary/30 rounded-2xl py-4 px-3 ${className}`}
      style={{
